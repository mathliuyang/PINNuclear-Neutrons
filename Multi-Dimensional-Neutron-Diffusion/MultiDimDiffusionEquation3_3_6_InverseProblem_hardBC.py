...
def phi_analytical(x):
    ...
    for n in range(1, 10):  # 选择一个足够大的上限来近似无穷级数
        ...
        phi_0 = lambda x_prime: 0.5 * (np.cos(2 * np.pi * x_prime / a) + 1)
...
# 训练模型
losshistory, train_state = model.train(epochs=500, callbacks=[variable])
# # 定义求解器
...
# 可视化
# 创建保存图像的文件夹
output_folder = "figure/算例3/散点图/6"
