...
# 定义数据
data = dde.data.PDE(geom, pde, [], num_domain=898, num_boundary=2, solution=phi_analytical, num_test=900)
# 定义神经网络
layer_size = [1] + [s] * l + [1]
activation = "tanh"
# 网络初始值权重采用高斯分布随机采样
...
for module in net.modules():
    if isinstance(module, torch.nn.Linear):
        module.bias.data = torch.tensor([3.5], requires_grad=True)
...
# 确保文件夹路径存在
output_folder = "model/算例2/model2"
os.makedirs(output_folder, exist_ok=True)

model_path = "model/算例2/model2.pth"
# 保存模型
